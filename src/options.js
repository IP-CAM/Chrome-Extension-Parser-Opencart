...
(async function () {
	...
	$banBtn.addEventListener('click', () => {
		...
		let settings = [...$settings].reduce((obj, el) => {				
				if( (el.value === '') && (el.id === 'apiurl' || el.id === 'apiusername' ||el.id === 'apikey' ) ){										
	...
	$getSettings.addEventListener('click', () => {			
		...
		$.ajax({
        url: settings.apiurl + "/index.php?route=api/products/getCategories&api_token=" + settings.apitoken,
        type: "post",
        data: {
            "product_id": "30",           
        ...
        success: function (data) {
            if(data.categories){
				...
				$category.append( "<option value='" +data.categories[i].category_id+ "'>" + data.categories[i].name + "</option> " );				
